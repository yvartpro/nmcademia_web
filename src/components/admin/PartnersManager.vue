<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <h3 class="text-base font-bold text-amber-400 font-display uppercase tracking-wider">Manufacturing Partners</h3>
    ...
    <div class="bg-slate-950 border border-gray-900 rounded-xl overflow-hidden shadow-2xl">
      <table class="w-full text-left text-xs">
        ...
        <tbody class="divide-y divide-gray-950 text-gray-300">
          <tr v-for="item in contentStore.manufacturingPartners" :key="item.id" class="hover:bg-slate-900/30">
            <td class="p-4 font-bold text-white">{{ item.name }}</td>
            <td class="p-4 text-amber-500 font-mono">{{ item.country }}</td>
          ...
          <tr v-if="contentStore.manufacturingPartners.length === 0">
            <td colspan="4" class="p-6 text-center text-gray-500">No partners found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="max-w-lg w-full bg-slate-950 border border-gray-800 rounded-2xl p-6 space-y-4 shadow-2xl relative">
        ...
        <form @submit.prevent="saveItem" class="space-y-4 text-xs">
          <div>
            <label class="block text-gray-400 uppercase tracking-wider font-bold mb-1">Company Name</label>
          ...
          <div>
            <label class="block text-gray-400 uppercase tracking-wider font-bold mb-1">Country</label>
            <input v-model="form.country" placeholder="e.g. USA, Europe" class="w-full bg-slate-900 border border-gray-800 rounded p-2 focus:border-amber-400 text-white" />
          </div>
          <div>
...
const handleDelete = async (id) => {
  if (confirm('Delete this partner?')) {
    await contentStore.adminDeleteManufacturingPartner(id);
  }
};
</script>
