<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex items-center justify-between">
      <h3 class="text-base font-bold text-[#008A20] font-display uppercase tracking-wider">Manufacturing Partners</h3>
    ...
    <div class="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-sm">
      <table class="w-full text-left text-xs">
        ...
        <tbody class="divide-y divide-zinc-100 text-zinc-600">
          <tr v-for="item in contentStore.manufacturingPartners" :key="item.id" class="hover:bg-[#F4F6F5]/60 transition">
            <td class="p-4 font-bold text-[#0A0F0D]">{{ item.name }}</td>
            <td class="p-4 text-[#008A20] font-mono font-semibold">{{ item.country }}</td>
          ...
          <tr v-if="contentStore.manufacturingPartners.length === 0">
            <td colspan="4" class="p-6 text-center text-zinc-400">No partners found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <!-- Form Modal -->
    <UiModal v-model="isModalOpen" :title="editingId ? 'Edit Partner' : 'New Partner'" subtitle="Global Partners">
      <form id="partner-form" @submit.prevent="saveItem" class="space-y-4 text-xs">
        <div class="adm-field">
        ...
        <div class="adm-field">
          ...
          <input v-model="form.country" placeholder="e.g. USA, Europe" class="adm-input" />
        </div>
        <div class="adm-field">
      ...
      <template #footer>
        ...
        <button type="submit" form="partner-form" class="adm-btn-primary" :disabled="saving">
          {{ saving ? 'Please wait…' : (editingId ? 'Save Changes' : 'Create') }}
        </button>
      </template>
    </UiModal>
    ...
    <UiConfirmModal
      ...
      message="This will permanently remove the manufacturing partner from the presentation."
...
const form = ref({ name: '', country: '', description: '', mediaAssetId: null });
...
const openModal = (item = null) => {
  if (item) {
    ...
    form.value = { name: '', country: '', description: '', mediaAssetId: null };
  }
...
const saveItem = async () => {
  ...
  try {
    if (editingId.value) {
      await contentStore.adminUpdateManufacturingPartner(editingId.value, form.value);
    } else {
      await contentStore.adminCreateManufacturingPartner(form.value);
    }
